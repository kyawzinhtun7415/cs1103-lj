...
public class TextMenu extends JMenu {
	...
	private JRadioButtonMenuItem left; // controls whether the text is justified left or not.
	...
	public TextMenu(DrawPanel owner) {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (s != null && s.trim().length() > 0) {
					try {
						...
						JOptionPane.showMessageDialog(panel, s + " is not a legal value.\n"
		...
		left.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				panel.getTextItem().setJustify(LEFT);;
				panel.repaint();
			}
		});
		add(change);
	...
	/**
	 * Create a menu containing 3 options for justifying the text left, right, or center
	 * @return
	 */
	private JMenu makeJustifyMenu() {
		ActionListener setJustifyLeftAction = new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				panel.getTextItem().setJustify(TextItem.LEFT);
				panel.repaint();
			}
		};
		ActionListener setJustifyRightAction = new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				panel.getTextItem().setJustify(TextItem.RIGHT);
				panel.repaint();
			}
		};
		ActionListener setJustifyCenterAction = new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				panel.getTextItem().setJustify(TextItem.CENTER);
				panel.repaint();
			}
		};
		JMenu menu = new JMenu("Justify Text");
		...
		left.addActionListener(setJustifyLeftAction);
		right.addActionListener(setJustifyRightAction);
		center.addActionListener(setJustifyCenterAction);
		menu.add(left);
		menu.add(right);
		menu.add(center);
		bg.add(center);
